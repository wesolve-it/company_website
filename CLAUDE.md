...
- **JavaScript**: Alpine.js is loaded exactly once per page via the CDN build (`cdn.jsdelivr.net/npm/alpinejs@3.x.x`). Do NOT additionally load `alpinejs` as an ES module — a second Alpine instance double-initializes all `x-data` trees and breaks the mobile navigation toggle. Alpine directives (`x-data`, `x-show`, etc.) are used inline in the HTML (mobile navigation, FAQ accordions, Referenzen filter).
